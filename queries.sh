...
echo "$($PSQL "select name from teams full join games on teams.team_id=games.game_id where year=2018 and round='Final'")"
...
echo "$($PSQL "select year,name from teams full join games on teams.team_id=games.game_id where round='Final' order by year")"
